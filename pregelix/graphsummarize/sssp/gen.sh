...
do
  ...
  do
	...
	#factor=`expr 1.0/(1.25-0.25*$factor)|bc`
...
do
  ...
  do
	...
	pregelixentry=$(echo "scale=3; $pregelixentry/1000"|bc)
        if [ "$giraphmementry" != "FAILED" ]; then
		giraphmementry=$(echo "scale=3; $giraphmementry/1000"|bc)
	fi
	
	if [ "$giraphoocentry" != "FAILED" ]; then
        	giraphoocentry=$(echo "scale=3; $giraphoocentry/1000"|bc)
	fi
