...
do
  if [ $line -eq 7 ] ; then
       #scale="1"
       scale=$(echo "scale=3; 2.62/$scalebase"|bc)
       num=`echo $pline|awk '{print $2}'`
       num=$(echo "scale=3; $num/$base"|bc)
       echo "$scale $num">>scaleup.txt
  fi
  if [ $line -eq 8 ] ; then
       #scale="1"
       scale=$(echo "scale=3; 7.037/$scalebase"|bc)
  ...
  if [ $line -eq 9 ] ; then
       #scale="2"
       scale=$(echo "scale=3; 13.45/$scalebase"|bc)
  ...
  if [ $line -eq 10 ] ; then
       scale=$(echo "scale=3; 16.6/$scalebase"|bc)
       #scale="4"
...
do
   ...
   do
     if [ $line -eq 7 ]; then
     	linestr=`echo "$linestr "`	
     else
     	if [ $line -eq 8 ]; then
        	baseg=`echo $gmline|awk '{print $2}'`
     	fi
     	if [ $line -eq $linenum ]; then
       		#scale=`echo $pline|awk '{print $1}'`
       		num0=`echo $gmline|awk '{print $2}'`
